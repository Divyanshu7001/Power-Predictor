...
export default function Analytics() {
  const [forecast, setForecast] = useState([]); // Initialize as an empty array, not null
  const [temp, setTemp] = useState([]);

  useEffect(() => {
    const fetchForecast = async () => {
      try {
        const resp = await Weather("asansol");
        setForecast(resp.forecast.forecastday);
        console.log("Forecast data:", resp.forecast); // Log the forecast data after setting it

        // Check if forecast data exists and map it to get temperatures
        if (resp.forecast && resp.forecast.forecastday) {
          const tempArray = resp.forecast.forecastday.map(
            (day) => day.day.avgtemp_c
          );
          setTemp(tempArray); // Set temperatures after successful mapping
        }
      } catch (error) {
    ...
    fetchForecast(); // Call the async function to fetch weather data
  ...
  const weeklyData = {
    ...
    datasets: [
      ...
      {
        label: "Temperature (°C)",
        data: temp, // Use the temperature data fetched from the API
  ...
  return (
    <section className="bg-gradient-to-br from-gray-900 via-gray-800 to-orange-900 min-h-screen pt-24">
      <div className="container mx-auto px-4 relative">
        <Link
          to="/dashboard"
          className="absolute right-5 bg-orange-500 hover:bg-orange-600 text-white font-bold py-2 px-4 rounded transition duration-300"
        >
          Dashboard
        </Link>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
          <div className="bg-gray-800 p-6 rounded-lg shadow-xl border-orange-500 border">
            <h2 className="text-2xl font-semibold text-orange-300 mb-4">
              Estimated Usage for Next 12 Months
            </h2>
            <Line
              data={monthlyData}
              options={{
          ...
          <div className="bg-gray-800 p-6 rounded-lg shadow-xl border-orange-500 border">
            <h2 className="text-2xl font-semibold text-orange-300 mb-4">
              Next 7 Days Usage and Temperature
            </h2>
            <Line
              data={weeklyData}
              options={{
